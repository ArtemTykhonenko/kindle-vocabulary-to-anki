 ...
 * Automatically falls back to direct client-side calls if the server-side proxy is unavailable.
 */
  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    while (!success && retries <= MAX_RETRIES) {
      try {
        ...
        if (!res.ok) {
          throw new Error(`Server lookup failed with status: ${res.status}`);
        }
        ...
        console.warn(`Local lookup proxy failed or unavailable (attempt ${retries + 1}), trying client fallback:`, error);
        
        if (retries >= MAX_RETRIES) {
          // Perform direct client-side lookup for this chunk as a fallback
          ...
          for (const item of chunk) {
            ...
            try {
              ...
              console.error("Client translation fetch failed:", err);
            }
            ...
            // Gentle delay to not spam direct API targets
            await sleep(200);
          }
